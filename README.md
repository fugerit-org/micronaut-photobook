...
[![Quality Gate Status](https://sonarcloud.io/api/project_badges/measure?project=fugerit-org_micronaut-photobook&metric=alert_status)](https://sonarcloud.io/summary/new_code?id=fugerit-org_micronaut-photobook)
[![Coverage](https://sonarcloud.io/api/project_badges/measure?project=fugerit-org_micronaut-photobook&metric=coverage)](https://sonarcloud.io/summary/new_code?id=fugerit-org_micronaut-photobook)
[![Docker images](https://img.shields.io/badge/dockerhub-images-important.svg?logo=Docker)](https://hub.docker.com/repository/docker/fugeritorg/micronaut-photobook/general)
...
```shell
java -jar target/micronaut-photobook-*.jar
```
...
```shell
./target/micronaut-photobook
```

Refer to [Micronaut Documentation](https://guides.micronaut.io/latest/micronaut-creating-first-graal-app-maven-java.html) for more informations.
...
```shell
docker build -t micronaut-photobook-jvm -f src/main/docker/Dockerfile.jvm .
```

Running the container :

```shell
docker run -it -p 8080:8080 --name micronaut-photobook-jvm micronaut-photobook-jvm
```
...
```shell
docker build -t micronaut-photobook-native -f src/main/docker/Dockerfile.native .
```

Running the container :

```shell
docker run -it -p 8080:8080 --name micronaut-photobook-native micronaut-photobook-native
```
